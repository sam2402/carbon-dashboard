...
def get_prompt(resource_emission_infos: list[ResourceEmissionInfo], advice_type: AdviceType) -> str:
    ...
    for resource_emission_info in resource_emission_infos:
        ...
        if advice_type == AdviceType.ENERGY_TYPE:
            prompt = f"Volvo server {resource_name} has a carbon emission value of {current_carbon_emission}g from the last week, provide advice on how to change energy sources in order to decrease carbon emissions. Below is a list of each zone's breakdown of electricity usage by percentage:\n"
            ...
            prompt = f"Volvo server {resource_name} is currently located at {location_name} and last week's carbon emission value is {current_carbon_emission}g, give some suggested locations in order to decrease carbon emissions.\n"
            ...
            prompt = f"Volvo server {resource_name} is currently using a {cpu_model} CPU with a power rating of {power_rating} watts and last week's carbon emission value is {current_carbon_emission}g, could you please give some exact suggested cpu types with lower power ratings but similar compatibility to replace the current one then it can decrease carbon emissions.\n"

        elif advice_type == AdviceType.COOLING_TYPE:
            prompt = f"Volvo server {resource_name} has a carbon emission value of {current_carbon_emission}g from the last week. Could you please suggest a suitable and efficient cooling type for the current server, along with an approximate cost estimation.\n"
