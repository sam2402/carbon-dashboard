...
def get_prompt(carbon_data):
    ...
    if calculated_future_carbon > clean_carbon_level:
        if server_energy_type in energy_types:
            ...
            for energy_type, energy_type_carbon_value in energy_types.items():
                ...
                if energy_type_carbon_value < energy_types[current_energy_type] and calculated_future_carbon < clean_carbon_level:
                    advice_energy_type = "Use of cleaner energy sources: " + energy_type + ". "
                    prompt = prompt + advice_energy_type
                    break
                elif energy_type == 'Nuclear Energy' and calculated_future_carbon > clean_carbon_level:
    ...
    if calculated_future_carbon > clean_carbon_level:
        if server_cooling_type in cooling_types:
            ...
            for cooling_type, cooling_type_carbon_value in cooling_types.items():
                ...
                if cooling_type_carbon_value < cooling_types[current_cooling_type] and calculated_future_carbon < clean_carbon_level:
                    advice_cooling_type = "Use of cleaner cooling type: " + cooling_type + ". "
                    prompt = prompt + advice_cooling_type
                    break
                elif cooling_type == 'Heat pipe cooling system (water-cooled or dry-cooled)' and calculated_future_carbon > clean_carbon_level:
    ...
    if calculated_future_carbon > clean_carbon_level:
        if server_configuration in server_configurations:
            ...
            for configuration, configuration_carbon_value in server_configurations.items():
                ...
                if configuration_carbon_value < server_configurations[configuration] and calculated_future_carbon < clean_carbon_level:
                    advice_server_configuration = "Change the CPU current using to: " + configuration + ". "
                    prompt = prompt + advice_server_configuration
                    break
                elif configuration == 'Intel Xeon E5-2660 v4' and calculated_future_carbon > clean_carbon_level:
